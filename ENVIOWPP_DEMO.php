...
function processarResultadoEnvio($conn, $result, $fatura, $telefone, $mensagem, $execId, $sessaoFinal, $modeloMensagem)
{
    ...
    try {
        if (isset($result['success']) && $result['success'] === true) {
            ...
            $logStmt->execute([
                ':fatura' => $cdFatura, ':contato' => $nomeCliente, ':celular' => $telefone,
                ':nome' => $nomeCliente, ':valor' => $fatura['VALOR'], ':data_vencimento' => $vencFormat,
                ':modelo_mensagem' => $modeloMensagem, ':exec_id' => $execId, ':payload_json' => $payloadJson
            ]);
            ...
            $logFalhaStmt->execute([
                ':fatura' => $cdFatura, ':contato' => $nomeCliente, ':celular' => $telefone,
                ':nome' => $nomeCliente, ':valor' => $fatura['VALOR'], ':data_vencimento' => $vencFormat,
                ':erro' => $erroMsg, ':modelo_mensagem' => $modeloMensagem, ':exec_id' => $execId,
...
function enviarMensagemComControleDeDuplicatas($conn, $fatura, $telefone, $mensagem, $execId, $sessionWpp, $modeloMensagem)
{
    ...
    try {
        ...
        // 3. SIMULA o envio da mensagem através da API
        debug_print("--- MODO DE DEMONSTRAÇÃO ---", [
            'Ação' => 'Simulação de Envio de Mensagem',
            'Sessão' => $sessaoFinal,
            'Telefone' => $telefone,
            'Mensagem' => $mensagem
        ], 'INFO');
        $result = ['success' => true, 'response' => ['message' => 'Simulação de envio bem-sucedida.']];
...
try {
    ...
    $feriados = [
        ...
        '15/11/2025', '20/11/2025', '24/12/2025', '25/12/2025', '31/12/2025',
    ];
    $dataHoje = date('d/m/Y');
