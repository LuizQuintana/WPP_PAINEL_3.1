...
if (file_exists(__DIR__ . "/config/function_auth_fortics.php")) {
    include_once __DIR__ . "/config/function_auth_fortics.php";
...
try {
    ...
    foreach ($reguasAtivas as $regua) {
        ...
        $inicioPermitido = clone $horaRegua; // Clonar para não modificar o original
        $fimPermitido = clone $horaRegua;
        $fimPermitido->add(new DateInterval('PT2H')); // Adiciona 2 horas
        ...
        foreach ($faturas as $fatura) {
            ...
            try {
                ...
                if ($sucessoEnvio) {
                    $payloadJson = json_encode($dataEnvioMsg, JSON_UNESCAPED_UNICODE);

                    $logQuery = "INSERT INTO LOG_ENVIOS 
                                (FATURA, CONTATO, CELULAR, NOME, VALOR, DATA_VENCIMENTO, DATA_ENVIO, MODELO_MENSAGEM, EXEC_ID, PAYLOAD_JSON) 
                    ...
                    $logQuery = "INSERT INTO LOG_ENVIOS_FALHA 
                                (FATURA, CONTATO, CELULAR, NOME, VALOR, DATA_VENCIMENTO, DATA_ENVIO, ERRO, MODELO_MENSAGEM, EXEC_ID, PAYLOAD_JSON) 
                    ...
                    if (strpos($erroMsg, 'Recipient is not a valid WhatsApp user') !== false) {
                        ...
                        (FATURA, CONTATO, CELULAR, NOME, VALOR, DATA_VENCIMENTO, DATA_ENVIO, ERRO, MODELO_MENSAGEM, STATUS, EXEC_ID, PAYLOAD_JSON) 
                        VALUES (:fatura, :contato, :celular, :nome, :valor, :data_vencimento, NOW(), :erro, :modelo_mensagem, :status, :exec_id, :payload_json)";
                        ...
                        $error_contact_stmt->bindParam(':payload_json', $payloadJson);
